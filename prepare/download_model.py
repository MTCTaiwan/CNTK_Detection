...
def download_model(model_file_name, model_url, model_dir):
    filename = os.path.join(model_dir, model_file_name)
    if not os.path.exists(filename):
        print('[INFO] Downloading model from ' + model_url + ', may take a while...')
        urlretrieve(model_url, filename)
        print('[SUCCESS] Saved model as ' + filename)
    else:
        print('[SUCCESS] CNTK model already available at ' + filename)
    
def download_model_by_name(model_name, model_dir):
    ...
    if model is None:
        ...
        print("[ERROR] '%s' is not exist" % model_dir)
    else:
        download_model(model_name + '.model', model[2], model_dir)
...
if __name__ == "__main__":
    ...
    if len(args) != 2:
        ...
        if model_name == 'list':
            ...
            download_model_by_name(model_name, os.path.basename(__file__))
